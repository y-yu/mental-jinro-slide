...
\begin{frame}[fragile]
  ...
  \begin{center}
    \begin{tikzpicture}
      \calloutquote[author={\includegraphics[width=1cm]{./img/bird2x.png}},width=10cm,position={(-1.1,-0.5)},fill=green!30,rounded corners]{\huge\textbf{ゲームマスター}を消そう！}
    \end{tikzpicture}
  \end{center}
...
\begin{frame}
  \centering
  {\Huge Mental Jinroを支える技術 \\ \uncover<2->{\textbf{コミットメント}}}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{center}
    ...
    \uncover<7->{
      \begin{tikzpicture}
        \calloutquote[author={},width=6.5cm,position={(1.1,-0.3)},fill=blue!30,rounded corners]{ボブがアリスの主張を信じない？}
      \end{tikzpicture}
    }
  \end{center}
\end{frame}
